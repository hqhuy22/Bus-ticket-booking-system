...
export function validateScheduleTripInput(data: any): ValidationResult {
  const errors: any[] = [];
  const value: any = {};
  ...
  if (!route_id) errors.push({ field: 'route_id', message: 'route_id is required' });
  else value.route_id = route_id;

  if (!bus_id) errors.push({ field: 'bus_id', message: 'bus_id is required' });
  else value.bus_id = bus_id;
  ...
  else value.departure_time = new Date(String(departure_time)).toISOString();

  if (!arrival_time) errors.push({ field: 'arrival_time', message: 'arrival_time is required' });
  else if (!isISODate(arrival_time))
    errors.push({ field: 'arrival_time', message: 'arrival_time must be a valid ISO date' });
  else value.arrival_time = new Date(String(arrival_time)).toISOString();
  ...
  else value.base_price = Number(base_price);
...
export function validateSearchInput(query: any): ValidationResult {
  const errors: any[] = [];
  const value: any = {};
  ...
  if (query.origin !== undefined) {
    ...
    else value.origin = String(query.origin).trim();
  }
  ...
  if (query.destination !== undefined) {
    ...
    else value.destination = String(query.destination).trim();
  }

  if (query.date !== undefined) {
    if (!isISODate(query.date)) errors.push({ field: 'date', message: 'date must be an ISO date' });
    else value.date = new Date(String(query.date)).toISOString();
  }
  ...
  if (query.minPrice !== undefined) {
    ...
    else value.minPrice = Number(query.minPrice);
  }
  ...
  if (query.maxPrice !== undefined) {
    ...
    else value.maxPrice = Number(query.maxPrice);
  }
  ...
  if (query.departureStart !== undefined) value.departureStart = String(query.departureStart);
  if (query.departureEnd !== undefined) value.departureEnd = String(query.departureEnd);
  if (query.busType !== undefined) value.busType = String(query.busType);
  if (query.amenities !== undefined) value.amenities = query.amenities;
...
export function validateSeatConfigInput(data: any): ValidationResult {
  const errors: any[] = [];
  const value: any = {};
  ...
  else value.seat_code = String(seat_code).trim();
  ...
  if (price_modifier !== undefined) {
    ...
    else value.price_modifier = Number(price_modifier);
  }

  if (is_active !== undefined) value.is_active = Boolean(is_active);
...
export function validateScheduleQuery(query: any): ValidationResult {
  const errors: any[] = [];
  const value: any = {};
  ...
  else value.start = new Date(String(query.start)).toISOString();

  if (!query || !query.end)
    errors.push({ field: 'end', message: 'end query param is required (ISO date)' });
  else if (!isISODate(query.end)) errors.push({ field: 'end', message: 'end must be an ISO date' });
  else value.end = new Date(String(query.end)).toISOString();

...
import { BookingWithDetails } from '../models/booking';
import QRCode from 'qrcode';

export type TicketData = {
  reference: string;
  passengers: { full_name: string; seat_code: string }[];
  origin?: string;
  destination?: string;
...
export default class TicketService {
  static async generateQRCode(data: string): Promise<string> {
    try {
      ...
      return await QRCode.toDataURL(data, { margin: 1, width: 200 });
    } catch (err) {
      const e = err instanceof Error ? err : new Error(String(err));
      throw new Error(`Failed to generate QR code: ${e.message}`);
    }
  }
  static async getTicketData(bookingId: string): Promise<TicketData> {
    const b = await BookingService.getBookingDetails(bookingId);
  ...
  static async generateTicketHTML(booking: BookingWithDetails): Promise<string> {
    // Prepare data
  ...
  static async sendTicketEmail(bookingId: string): Promise<void> {
    try {
      ...
      if (!recipient) throw new Error('No contact email for booking');

...
function redisKey(tripId: string, seatId: string) {
...
export default class SeatLockService {
  ...
  static async lockSeats(
    ...
    if (!seatIds || seatIds.length === 0) return true;

    const ttl = Math.max(1, Math.floor(durationMinutes * 60));

    try {
      const r = redis;
      if (r && (await ensureConnected())) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (res === 'OK') succeededKeys.push(keys[i]);
        }
        ...
        if (succeededKeys.length > 0) await r.del(...succeededKeys);
        return false;
      }
      ...
      try {
        ...
        const updated = res.rows.map((r: any) => r.seat_id);
   ...
   */
  static async unlockSeats(tripId: string, seatIds: string[], sessionId: string): Promise<void> {
    ...
    try {
      ...
      if (r && (await ensureConnected())) {
        ...
        for (let i = 0; i < keys.length; i++) {
          const v = vals[i];
          if (v && (sessionId ? v === sessionId : true)) toDelete.push(keys[i]);
        }
      ...
      console.info(
        'unlockSeats: Redis unavailable, falling back to DB-only unlock (will verify lock_owner)',
      );
      const client = await getClient();
      try {
        await client.query('BEGIN');
        await client.query(
          `UPDATE seat_statuses SET locked_until = NULL, lock_owner = NULL WHERE trip_id = $1 AND seat_id = ANY($2) AND lock_owner = $3`,
  ...
  static async checkSeatAvailability(
    tripId: string,
    seatIds: string[],
  ): Promise<{ seatId: string; available: boolean; lockedBy?: string }[]> {
    if (!seatIds || seatIds.length === 0) return [];

    try {
      const result: { seatId: string; available: boolean; lockedBy?: string }[] = [];

      // First check Redis locks if available
      const r = redis;
      const redisAvailable = !!r && (await ensureConnected());
      const keys = seatIds.map((s) => redisKey(tripId, s));
      let redisVals: any[] = [];
      if (redisAvailable) {
      ...
      const dbRes = await query(q, [tripId, seatIds]);
      const dbMap: Record<string, any> = {};
      for (const row of dbRes.rows) dbMap[row.seat_id] = row;
  ...
  static async getLockedSeats(tripId: string): Promise<string[]> {
    try {
      ...
      if (r && (await ensureConnected())) {
        ...
        // extract seatIds
        return found.map((k) => k.split(':').pop() as string);
      }
      ...
      const res = await query(q, [tripId]);
      return res.rows.map((r: any) => r.seat_id as string);
    } catch (err) {

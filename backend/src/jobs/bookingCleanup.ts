 ...
 */
export async function expireOldBookings(): Promise<void> {
  ...
  try {
    ...
    const bookingIds: string[] = res.rows.map((r: any) => r.id);
    let expiredCount = 0;
    ...
    for (const id of bookingIds) {
      try {
        ...
        const upd = await client.query(
          `UPDATE bookings SET status = 'expired' WHERE id = $1 RETURNING id`,
          [id],
        );
        ...
        console.info(
          `[bookingCleanup] Expired booking ${id}, released seats: ${releaseRes.rows.length}`,
        );
      } catch (err) {
 ...
 */
export function setupBookingCleanupJob(): void {
  ...
  setInterval(() => {
    void expireOldBookings().catch((e) =>
      console.error('[bookingCleanup] Scheduled run failed:', e),
    );
  }, intervalMs);
...
export default setupBookingCleanupJob;

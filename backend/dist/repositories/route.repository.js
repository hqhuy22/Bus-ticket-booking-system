...
class RouteRepository {
    ...
    static async createRoute(data) {
        ...
        try {
            ...
            const r = await client.query(`INSERT INTO routes (operator_id, origin, destination, distance_km, estimated_minutes) VALUES ($1,$2,$3,$4,$5) RETURNING *`, [
            ...
            if (data.stops && data.stops.length > 0) {
                for (const s of data.stops) {
                    const res = await client.query(`INSERT INTO route_stops (route_id, stop_name, stop_order, pickup_time_offset, stop_type, is_active) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [
                        route.id,
                        s.stop_name,
                        s.stop_order,
    ...
    static async updateRoute(id, data) {
        ...
        if (sets.length === 0) {
            const res = await (0, db_1.query)(`SELECT * FROM routes WHERE id = $1`, [id]);
            return res.rows[0];
        }
        params.push(id);
    ...
    static async getRouteWithStops(id) {
        const r = await (0, db_1.query)(`SELECT * FROM routes WHERE id = $1`, [id]);
        const route = r.rows[0];
        if (!route)
            return undefined;
        const s = await (0, db_1.query)(`SELECT * FROM route_stops WHERE route_id = $1 ORDER BY stop_order ASC`, [
            id,
        ]);
        return { ...route, stops: s.rows };
    }
    // Add a stop to a route. This will shift existing stops with >= stop_order up by 1.
    static async addRouteStop(routeId, stop) {
        const client = await (0, db_1.getClient)();
        try {
            await client.query('BEGIN');
            // shift existing stops
            await client.query(`UPDATE route_stops SET stop_order = stop_order + 1 WHERE route_id = $1 AND stop_order >= $2`, [routeId, stop.stop_order]);
            const res = await client.query(`INSERT INTO route_stops (route_id, stop_name, stop_order, pickup_time_offset, stop_type, is_active) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [
                routeId,
                stop.stop_name,
                stop.stop_order,
    ...
    static async updateRouteStop(stopId, data) {
        const client = await (0, db_1.getClient)();
        try {
            await client.query('BEGIN');
            ...
            if (newOrder !== oldOrder) {
                if (newOrder < oldOrder) {
                    await client.query(`UPDATE route_stops SET stop_order = stop_order + 1 WHERE route_id = $1 AND stop_order >= $2 AND stop_order < $3 AND id <> $4`, [routeId, newOrder, oldOrder, stopId]);
                }
                else {
                    await client.query(`UPDATE route_stops SET stop_order = stop_order - 1 WHERE route_id = $1 AND stop_order > $2 AND stop_order <= $3 AND id <> $4`, [routeId, oldOrder, newOrder, stopId]);
                }
            }
            const fields = [];
            const params = [];
    ...
    static async deleteRouteStop(stopId) {
        ...
        try {
            ...
            await client.query(`UPDATE route_stops SET stop_order = stop_order - 1 WHERE route_id = $1 AND stop_order > $2`, [routeId, oldOrder]);
            await client.query('COMMIT');
            return true;
        }
        catch (err) {
...
exports.default = RouteRepository;
